...
export default function AdminRahbariyatPage() {
  ...
  const [selected, setSelected] = useState<RahbariyatMember | null>(null);
  const [isAdd, setIsAdd] = useState(false);
  const [isEdit, setIsEdit] = useState(false);
  const [isDelete, setIsDelete] = useState(false);

  useEffect(() => { initializeData(); setList(getRahbariyat()); }, []);
  ...
  const handleAdd = () => {
    const item = addRahbariyatMember(formData);
    setList([item, ...list]);
    setIsAdd(false);
  ...
  const handleEdit = () => {
    if (!selected) return;
    const updated = updateRahbariyatMember(selected.id, formData);
  ...
  const handleDelete = () => {
    if (!selected) return;
    deleteRahbariyatMember(selected.id);
    ...
    <div>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-600">Yoshlar parlamenti rahbariyat tarkibini boshqarish</p>
        </div>
        <Dialog open={isAdd} onOpenChange={setIsAdd}>
          ...
          <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
            <DialogHeader><DialogTitle>Yangi rahbariyat a'zosi qo'shish</DialogTitle></DialogHeader>
            <FormFields />
            <DialogFooter>
