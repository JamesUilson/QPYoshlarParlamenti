    ...
    <main className="min-h-screen bg-gray-50 pb-16">
      <section className="bg-[#0047AB] text-white py-10">
        <div className="container mx-auto px-4">
          <h1 className="text-2xl font-bold mb-1">{tr("kengashi-page-title")}</h1>
          <p className="text-sm text-white/80">{tr("kengashi-page-desc")}</p>
        </div>
      </section>
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-1 min-w-0">
            <div className="bg-white border border-gray-200 rounded-sm p-5 mb-6">
              <h2 className="text-lg font-bold mb-3">{tr("kengash-haqida")}</h2>
            ...
            <h2 className="text-xl font-bold mb-4">{tr("kengash-tarkibi")}</h2>
            {members.length === 0 ? (
                ...
                {members.map((m) => (
                  <div key={m.id} className="bg-white border border-gray-200 rounded-sm overflow-hidden hover:border-[#0047AB] transition">
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-xs text-[#0047AB] leading-snug">{getLocalized(m, "position", lang)}</p>
                      {m.description && <p className="text-xs text-gray-500 mt-2 line-clamp-2">{getLocalized(m, "description", lang)}</p>}
                    </div>
                  </div>
                ))}
              </div>
